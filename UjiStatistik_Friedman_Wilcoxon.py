...
df = pd.read_excel("Data_LLM as a Judge_GPT5.5_Gemini3.6.xlsx",
                   sheet_name="Skor Final", header=1)
df.columns = ['ID_Q','Pertanyaan','Sub_Bidang','Teknik',
              'RP','AI','KR','KC','KO','Rata_rata']
df = df.dropna(subset=['Teknik','RP','AI','KR','KC','KO','Rata_rata']).reset_index(drop=True)
...
for dim, label in zip(DIMS, DIM_LABELS):
    mat = matrices[dim]
    groups = [mat[t].values for t in TEKNIK] #mengambil data per teknik
    stat, p = stats.friedmanchisquare(*groups)
    n = len(mat)
    df_val = len(TEKNIK) - 1 #p-value degrees of freedom
...
for (t1, t2) in pairs:
    ...
    try:
        W, p = stats.wilcoxon(x, y, alternative='two-sided') #Hipotesis
    except Exception as e:
        W, p = np.nan, np.nan #Jika error hasilnya NaN 

    mean_diff = np.mean(x) - np.mean(y) #Rata-rata 
    sig_bonf = p < alpha_bonferroni if not np.isnan(p) else False #cek Signifikan
